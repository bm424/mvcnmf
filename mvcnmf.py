...
class MVCNMF:
    ...
    def grad_a(self, X, A, S):
        ...
        if not np.isclose(np.linalg.det(self.Z(A, mean)), 0):
            geometric_part = self.tau * np.square(np.linalg.det(self.Z(A, mean)))* self.U.dot(self.B.T).dot(np.linalg.inv(self.Z(A, mean)).T)
        else:
            geometric_part = 0.
    ...
    def fit(
        ...
        for iteration in iterator:
            alpha = self.get_alpha(X, A, S, learning_rate, 1, learning_rate, scaling, learning_tolerance)
            A = self.A_new(X, A, S, alpha)
            A_bar = self.augment(A)
            beta = self.get_beta(X_bar, A_bar, S, learning_rate, 1, learning_rate, scaling, learning_tolerance)
    ...
    def get_alpha(self, X, A, S, alpha, m, scaling, learning_tolerance, max_iter=15):
        ...
        condition = learning_tolerance * scaling * alpha * np.sum(np.dot(self.grad_a(X, A, S).T, A_new - A))
        alpha_new = alpha * scaling
        if f_new - f_old <= condition or m > max_iter:
            return alpha_new
        else:
            return self.get_alpha(X, A, S, alpha_new, m + 1, scaling, learning_tolerance, max_iter=max_iter)
    ...
    def get_beta(self, X, A, S, beta, m, scaling, learning_tolerance, max_iter=15):
        ...
        f_old = self.objective(X, A, S)
        condition = learning_tolerance * scaling * beta * np.sum(np.dot(self.grad_s(X, A, S).T, S_new - S))
        beta_new = beta * scaling
        if f_new - f_old < condition or m > max_iter:
            return beta_new
        else:
            return self.get_beta(X, A, S, beta_new, scaling, learning_tolerance, max_iter=max_iter)
